...
use App\Models\{Task, Project, Space, Role, Comment};
...
class TaskController extends Controller
{
    ...
    public function show(Space $space, Project $project, Task $task)
    {
        $this->authorizeRole($space, ['Admin', 'Project Manager', 'Member']);

        if ($project->id !== $task->project_id || $project->space_id !== $space->id) {
            abort(404, 'Task does not belong to this project or space.');
        }

        // Load comments with user info
        $task->load('comments.user');

        return view('tasks.show', compact('space', 'project', 'task'));
    }

    public function storeComment(Request $request, Space $space, Project $project, Task $task)
    {
        $this->authorizeRole($space, ['Admin', 'Project Manager', 'Member']);
        ...
        if (!$pivot) {
            abort(403, 'You must be a member of this space to comment.');
        }
        ...
        return redirect()->route('spaces.projects.tasks.show', ['space' => $space->id, 'project' => $project->id, 'task' => $task->id])
