        ...
        <div class="col-md-4">
          <div class="card mb-4 shadow-sm">
            <div class="card-body">
              ...
              <p class="card-text">{{ $space->description ?? 'No description' }}</p>
              <a href="{{ route('spaces.show', $space->id) }}" class="btn btn-outline-primary">
                <i class="bi bi-eye"></i> View Projects
              </a>
            <a href="{{ route('spaces.edit', $space->id) }}" class="btn btn-outline-secondary me-2">
                <i class="bi bi-pencil"></i> Edit
            </a>
            <form action="{{ route('spaces.destroy', $space->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this space?');">
                @csrf
                @method('DELETE')
